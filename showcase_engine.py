...
"""
RingNode AI - Showcase Engine v3.2  (Ghost Punch Anti-Cheat Mimarisi)

SORUN: Laptop kamerası titremeyi 'Direkt'/'Kroşe' olarak sınıflandırıyor → Ghost Punches.

ÇÖZÜM: 3 Katmanlı Bariyer (Gate) Sistemi
─────────────────────────────────────────────────────────────────────
  Gate 1 │ Visibility Check  │ Bilek landmark görünürlüğü < %60 →
         │                   │ model çağrılmadan kare 'Idle' sayılır.
─────────────────────────────────────────────────────────────────────
...
OOP Tasarım Kararları:
  ...
  - HitDetector         → Gate 2 + Gate 3 + combo yönetimi
...
# ─── 3 BARİYER EŞİK DEĞERLERİ ────────────────────────────────────────────────
VISIBILITY_THRESHOLD  = 0.60   # Gate 1 : MediaPipe bilek görünürlük alt sınırı
...
# Kamera yönü = negatif Z ekseni; pozitif X = kişinin solu.
#
...
class FighterState:
    ...
    def __init__(self, isim: str, renk: tuple, baslangic_hp: int = MAX_HP):
        ...
        # ── Gate Z/X: 3D Vektörel Hareket Takibi (World Landmark) ──────────────
        ...
        # Saldırı sınıfındayken reddedilen her hit buraya yazılır.
        # draw_gate_debug overlay'inde gösterilir; prod'da sıfırlama maliyeti yok.
    ...
    def update_wrist_3d(self, landmarks_3d) -> None:
        ...
        Z atılımı  → delta_z = curr_z − prev_z negatif olmalı (kameraya geliyor).
                     max(0, −delta_z) ile yalnızca ileri yönü yakalarız.

        X savrulması → abs(delta_x), Kroşe'nin yatay dönüşünü ölçer.

        Her iki bilek de hesaplanır; ikisinden büyük olan rolling tampona yazılır.
        """
    ...
    def reset_on_lost(self) -> None:
        """Sporcu görüntüden kaybolduğunda pipeline'ı sıfırlar."""
        self.kuyruk.clear()
        ...
        # 3D vektör tamponlarını da temizle
...
class PredictionPipeline:
    ...
    def _infer(self, kuyruk: deque) -> tuple[str, float]:
        ...
        # Hızlar: dt tavan 0.100 s + max ~8°/frame → ±80°/s; ±500 çok
        # konservatif ama gerçek yumrukları kesmez, saçma patlamaları keser.
    ...
    def process(self, sporcu: FighterState, landmarks_2d) -> tuple[str, float]:
        """
        landmarks_2d: MediaPipe 2D landmark listesi (visibility erişimi için gerekli)
        Döner: (sinif_adi, guven_skoru)
        """

        # ── GATE 1: Görünürlük Kontrolü ──────────────────────────────────────
        ...
        if vis_l < VISIBILITY_THRESHOLD or vis_r < VISIBILITY_THRESHOLD:
            # Bilek(ler) yeterince görünmüyor → gürültü riski yüksek, güvenli yol: Idle
        ...
        # Önceki sürümde yalnızca "Idle" temizliyordu.
        # "Savunma" temizlemezse eski saldırı oyları 9 frame birikip
        # sonraki küçük bir hareketi otomatik saldırıya dönüştürüyordu.
        # Kural: Saldırı dışı her kesin sonuç geçmişi sıfırlar.
        ...
        # Güvensiz frame'ler birikerek gelecek saldırıya avantaj sağlamasın.
...
# ═══════════════════════════════════════════════════════════════════════════════
# BÖLÜM 5: HitDetector  —  Gate 2 + Gate 3 + Combo Yönetimi
# ═══════════════════════════════════════════════════════════════════════════════
class HitDetector:
    """
    Hasar (hit) kararını ve yan etkilerini (HP düşürme, combo, floating text)
    yönetir. PredictionPipeline çıktısını alır, son iki bariyerden geçirir.
    ...
    def check_and_apply(
        ...
        Bariyer sırası (önce ucuz kontroller, sonra 3D hesaplar):
          ...
          6. Gate Z  — Sınıfa özel 3D vektör (Direkt→Z atılımı, Kroşe→X savrulması)
        """
        ...
        if savunan.mevcut_sinif in SAVUNMA_BLOK:
            saldiran.hit_reject_reason = "Bloklandı: Savunma"
            return False
        ...
        if saldiran.peak_wrist_speed < WRIST_SPEED_THRESHOLD:
            saldiran.hit_reject_reason = "Reddedildi: Hız Yetersiz"
            return False

        # ── Gate Z: Sınıfa Özel 3D Vektör Doğrulaması ────────────────────────
        # Direkt → Kameraya doğru Z atılımı zorunlu.
        #   El-yüz teması da hızlı olabilir ama baskın yön Y'dir (yukarı), Z değil.
        if sinif == "Direkt":
            if saldiran.peak_z_thrust < Z_THRUST_THRESHOLD:
                saldiran.hit_reject_reason = "Reddedildi: Z-Atılımı Yok"
                return False

        # Kroşe → Yatay X savrulması zorunlu; el-yüz teması baskın X'e sahip değil.
        elif sinif == "Krose":
            if saldiran.peak_x_sweep < X_SWEEP_THRESHOLD:
                saldiran.hit_reject_reason = "Reddedildi: X-Savrulması Yok"
                return False

        # Aparkat → Mevcut hız bariyeri yeterli; Y-ekseni kontrolü sonraki sürüme bırakıldı.
        ...
        # Eski yumruğun yüksek peak_z/x değerleri deque(maxlen=15) içinde
        # 15 frame daha yaşar. Bu sürede hit_armed sıfırlanıp tekrar True
        # olursa, stale peak değerleri Gate Z/X'i otomatik geçirir.
        # Sıfırlama ile bir sonraki hareketin kendi verisini sıfırdan kazanması
        # zorunlu hale gelir.
...
def draw_gate_debug(frame, sporcu: FighterState, w: int, h: int, taraf: str):
    """
    Geliştirme overlay'i — 5 satır bilgi:
      Satır 1: G2 Genel bilek hızı (2D)
      Satır 2: GZ Z-atılımı (Direkt için)
      Satır 3: GX X-savrulması (Kroşe için)
      Satır 4: G3 State Machine (hit_armed)
      Satır 5: Son red sebebi (saldırı sınıfındayken kırmızı yazı)
...
# ═══════════════════════════════════════════════════════════════════════════════
def main():
    print("🥊 RingNode AI v3.2 — RCA Düzeltmeleri (RC-1/3/4/6) Aktif")
    print(f"   Gate 1 : Visibility    < {VISIBILITY_THRESHOLD}")
    ...
    with vision.PoseLandmarker.create_from_options(options) as landmarker:
        while cap.isOpened():
            ...
            # Taban 0.015 s → max 67 fps varsayımı; eğitim ~30fps ile tutarlı.
            # Tavan 0.100 s → sistem donmalarında velocity spike'ı engeller.
            # np.clip float döndürür; tip tutarlılığı için açık cast.
            ...
            if result.pose_landmarks:
                # Oyuncuları sol kalça X konumuna göre sırala (P1=sol, P2=sağ)
                pairs = sorted(
                    zip(result.pose_landmarks, result.pose_world_landmarks),
                    key=lambda p: p[0][SOL_KALCA_ID].x,
                )

                for idx, (l2d, l3d) in enumerate(pairs[:2]):
                    sporcu = [sp1, sp2][idx]

                    # Gate 2: 2D normalize bilek hızını güncelle
                    sporcu.update_wrist_speed(l2d)

                    # Gate Z/X: 3D world landmark'lardan vektörel hareketi güncelle
            ...
            # ── Hit Detection (Gate 2 + Gate 3 burada uygulanır) ─────────────
            ...
            cv2.imshow("RingNode AI — ARCADE v3.0", frame)
